...
function App() {
  ...
  const {
    fetchedData, 
    setFetchedData
  } = useFetch(fetchAvailablePlaces, 'user-places')
  ...
  async function handleSelectPlace(selectedPlaces) {
    ...
    try{
      await updateUserPlaces([selectedPlaces, ...fetchedData])
    }catch(err){
  ...
  const handleRemovePlace = useCallback(async function handleRemovePlace() {
    ...
    await updateUserPlaces(fetchedData.filter(place => place.id != selectedPlace.current.id))
